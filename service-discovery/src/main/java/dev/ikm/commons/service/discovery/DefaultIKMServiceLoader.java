...
/**
 * The DefaultIKMServiceLoader class is responsible for loading and managing service loaders
 * for pluggable services. It implements the DefaultIKMServiceLoader interface and also serves
 * as a PluginLifecycleListener to receive notifications about the addition or removal of
 * plugin layers.
 */
public class DefaultIKMServiceLoader implements IKMServiceLoader {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultIKMServiceLoader.class);
    ...
    public <S> ServiceLoader<S> loader(Class<S> service) {
        ensureUses(service);
        return ServiceLoader.load(DefaultIKMServiceLoader.class.getModule().getLayer(), service);
    }
        ...
        if (classLoaders == null) {
            ...
            try {
                if (classLoaders == null) {
                    ...
                    classLoaderSet.add(DefaultIKMServiceLoader.class.getClassLoader());
